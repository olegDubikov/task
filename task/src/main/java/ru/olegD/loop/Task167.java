...
public class Task167 {
    public static void loop(int[] first, int[] second, int[] third) {
        ...
        joiner(second);
        joiner(third);
        System.out.println("Выше 4.5: " + count(avg(first), avg(second), avg(third))
                + ", наивысшая: "
                + Math.max(avg(first), Math.max(avg(second), avg(third))));
    }
...
/*
Посчитать средний балл оценок учеников за год
Ниже представлен метод loop(int[] first, int[] second, int[] third),
который принимает 3 массива целых положительных чисел - оценки 3 учеников по 5 предметам за год.
Задание: написать код, который подсчитывает среднюю годовую оценку каждого ученика по 5 предметам.
Результат вывести в отдельной строке для каждого ученика в виде:
"Оценки: х1 х2 х3 х4 х5, средняя: у".
После построчного вывода оценок для каждого ученика вывести в отдельной строке количество учеников,
у которых среднегодовая оценка выше 4.5, и наивысшую среднегодовую оценку в виде:
"Выше 4.5: х, наивысшая: у".
